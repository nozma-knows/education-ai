...
const SearchBar = ({ setSearch }: { setSearch: (search: string) => void }) => {
  return (
    <div className="flex flex-1 h-full items-center gap-4 px-4 bg-opacity-40 bg-black rounded-lg border-2 border-transparent hover:border-white focus-within:border-white">
      <BsSearch className="text-2xl text-gray-400" />
      <input
        className="flex w-full h-full text-lg bg-transparent outline-none text-white"
        placeholder="Search..."
  ...
  return (
    <div className="flex w-full gap-4">
      ...
      <Button
        ...
        className="flex h-full bg-black bg-opacity-40 px-4 py-2 rounded-lg text-white border-2 border-transparent hover:border-white focus-within:border-white"
      />
    </div>
  );
};
